...
test.describe('AssetBridge Application', () => {
  ...
  /**
   * Test: Application displays correct content
   *
   * This test verifies that:
   * 1. The main heading is visible
   * 2. The subtitle is displayed
   * 3. The hello world message loads from API
   * 4. Tech stack information is shown
   */
  test('should display correct content', async ({ page }) => {
    ...
    /**
     * Assertion: Main Heading
     *
     * Checks that "AssetBridge" heading is visible.
     * Uses role='heading' for accessibility-first testing.
     */
    const heading = page.getByRole('heading', { name: 'AssetBridge' });
     ...
     */
    const subtitle = page.getByText('Procurement and Asset Management System');
    await expect(subtitle).toBeVisible();

    /**
     * Assertion: Loading State
     *
     * Initially, the loading message should appear.
     * This tests that the loading state works.
     */
    const loadingMessage = page.getByText('Loading...');
    // Note: This might not be visible if API is fast
    // await expect(loadingMessage).toBeVisible({ timeout: 1000 });

    /**
     * Assertion: Hello World Message
     *
     * Waits for the API response and verifies the message appears.
     * This tests:
     * - API call succeeds
     * - Data is displayed correctly
     * - Loading state transitions to success state
     */
    const helloMessage = page.getByRole('heading', {
      name: 'Hello World from AssetBridge!',
    });
    await expect(helloMessage).toBeVisible({ timeout: 5000 });

    /**
     * Assertion: Timestamp
     *
     * Verifies that a timestamp is displayed.
     * The exact timestamp varies, so we just check it exists.
     */
    const timestamp = page.getByText(/Timestamp:/);
    await expect(timestamp).toBeVisible();

    /**
     * Assertion: Tech Stack Information
     *
     * Checks that all tech stack items are visible.
     */
    await expect(page.getByText(/Frontend: React \+ Vite/)).toBeVisible();
    await expect(page.getByText(/Backend: Node\.js \+ Express/)).toBeVisible();
    await expect(page.getByText(/Database: MongoDB Atlas/)).toBeVisible();
    await expect(page.getByText(/Monorepo: npm workspaces/)).toBeVisible();

    console.log('✅ All content displayed correctly');
  });
